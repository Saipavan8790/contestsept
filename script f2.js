...
function getTotalProfessions() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // If the profession exists in the professionCounts object, increment its count
    if (professionCounts[profession]) {
      professionCounts[profession]++;
    } else {
      // If the profession does not exist in the object, initialize its count to 1
      professionCounts[profession] = 1;
    }
  }
  for (const profession in professionCounts) {
    console.log(`${profession}: ${professionCounts[profession]}`);
  }
}